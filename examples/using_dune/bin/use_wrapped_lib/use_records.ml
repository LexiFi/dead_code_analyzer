...
let () =
  let _ = fun r -> r.Wrapped_lib.Records.used_single_unit in
  let _ = fun r -> ignore r.Wrapped_lib.Records.used_single_param in
  let _ = fun r ->
    ignore r.Wrapped_lib.Records.used_param;
    r.Wrapped_lib.Records.used_unit
  in
  ()
...
let () =
  ...
  let _ = fun r -> r.Records.externally_used_single_unit in
  let _ = fun r -> ignore r.Records.externally_used_single_param in
  let open Records in
  let _ = fun r ->
